...
| **Sidebar Panel** | Context-sensitive panel for personal journal content, community interactions, and bestie matching | Panel header with breadcrumb navigation, Main tab navigation (Your Journal, Community, Besties), Song cards, Review display, Comment section, Community reviews list, Bestie match list | Navigate through three layers, Switch between tabs, View and edit reviews, Add comments, View community content, Connect with besties, Share content, Close sidebar |
...
| **Settings View** | User preference and privacy configuration | Header bar, Settings categories, Toggle switches, Radio buttons, Dropdown menus, Setting descriptions, Privacy controls, Account verification | Toggle options, Select preferences, Adjust display settings, Manage notifications, Configure data options, Set privacy, Verify account, Return to previous screen |
| **Terms/Privacy View** | Legal information and data usage policies | Header bar, Section headers, Legal text, Data usage section, Last updated timestamp | Read legal information, Review data practices, Check update date, Return to previous screen |
| **Left Sidebar Navigation** | Global navigation access to all app sections | App branding, Navigation menu, Menu icons, Active indicator, Close button, Login/Register | Navigate to section, See current location, Close menu, Access all app areas, Create account, Login |
| **Bestie Matching View** | Find users with similar music taste | Header bar, League table, Similarity percentage, User profiles, Comparison details, Filter options | View matches, Check similarity, Explore profiles, See details, Filter results, Return to previous screen |
| **Onboarding View** | Introduction for new users | Welcome screens, Guest mode option, Registration form, Comparison flow, Progress indicators, Skip button | Learn app features, Start as guest, Create account, Compare rankings, Track progress, Skip ahead |y controls, Account verification | Toggle options, Select preferences, Adjust display settings, Set privacy, Verify account, Return to previous screen |
...
**Figure: Sidebar Panel - Your Journal Tab (Layer 2)**
